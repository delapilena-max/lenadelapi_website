...
def build_proposed_live_command(day: str, recipes: str = "") -> str:
    strategy_cmd = f"{PY} {STRATEGY_RUNNER} --date {day} --queue-limit 1"
    if recipes:
        strategy_cmd += f" --recipes {recipes}"
    return f"{strategy_cmd} && {PY} {ANALYTICS_SYNC} --apply"
...
def main() -> int:
    ...
    if not strategy_step["ok"]:
        payload = {
            ...
            "proposed_live_command": build_proposed_live_command(args.date, args.recipes),
            "safeguards": {
