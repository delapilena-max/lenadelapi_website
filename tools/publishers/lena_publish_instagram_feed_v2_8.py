...
def main():
    ...
    args = ap.parse_args()
    payload = json.loads(
        Path(args.payload).read_text(encoding="utf-8-sig")
    )
    platform = "Instagram Feed"

    # ── HARD GATE — checked before token, R2, and Graph API ──────────────
    gate = check_final_publish_approval(payload)
    if not gate.get("ok"):
        print(json.dumps(
            fail(platform, payload, gate["reason"]),
            indent=2,
        ))
        return 1

    if payload.get("media_type") not in ("photo", "image"):
        print(json.dumps(fail(platform, payload,
            "instagram_feed_connector_expects_photo"), indent=2))
        return 1
    valid = validate_config_for(platform, payload.get("media_type", ""))
    if not valid.get("ok"):
        print(json.dumps(fail(platform, payload,
            valid.get("reason", "config_invalid"), valid), indent=2))
        return 1
    cfg = valid["config"]
    ...
    try:
        ...
        if ig_id != _EXPECTED_IG_ID:
            print(json.dumps(fail(platform, payload, "ig_id_guard_failed",
                {"resolved_id": ig_id, "expected_id": _EXPECTED_IG_ID}), indent=2))
            return 1
        container = graph_post(
            f"/{ig_id}/media",
            {"image_url": media["media_url"], "caption": payload.get("caption", "")},
            cfg, platform=platform,
        )
        creation_id = container.get("id")
        if not creation_id:
            print(json.dumps(fail(platform, payload,
                "ig_media_container_missing_id", container), indent=2))
            return 1
        ready = wait_for_container(creation_id, cfg,
                                   platform=platform, timeout_seconds=300)
        if not ready.get("ok"):
            print(json.dumps(fail(platform, payload,
                "ig_container_not_ready", ready), indent=2))
            return 1
        ...
        print(json.dumps(success(
            platform, payload, media_id,
            permalink(media_id, cfg, platform=platform),
            {"container": container, "published": published, "media": media},
        ), indent=2))
        return 0
