...
# Drop this file at project root and run with: python run_reel_forge.py
# Python 3.8+
...
def run_life_engine_stage():
    try:
        import importlib
        # import the hook module and call its run_once function
        hook = importlib.import_module("nodes.ai_lady_instagram.life_engine_hook")
        # run_once returns an exit code; log non-zero return codes
        rc = hook.run_once()
        if rc != 0:
            print(f"[orchestrator] life_engine_hook returned {rc}", file=sys.stderr)
        else:
            print(f"[orchestrator] life_engine_hook completed successfully")
    except ModuleNotFoundError:
        # If the hook module isn't present, skip gracefully
