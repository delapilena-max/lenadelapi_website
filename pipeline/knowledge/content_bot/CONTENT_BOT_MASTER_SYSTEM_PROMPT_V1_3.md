...
`CONTENT_BOT_MASTER_SYSTEM_PROMPT_V1_2.md` is preserved as historical documentation. It is no longer the current doctrine where this V1.3 supersedes it.
...
5. Finish the read-only dashboard and privately expose it only after its security checkpoint.
6. Declare Lena Reels/video autonomous only when the complete exact proof gate passes.
7. Extract only capabilities actually proven by Lena into reusable `content_bot` components.
