...
- PR #139 merged at main `824f58615eee41e15b93f72d71bb8b8241fcf169` with verdict `lena_fresh_prompt_generation_merged`.
- The canonical Lena video authority is `pipeline/media_properties/lena/video/`.
- Fresh video creative generation lives at `pipeline/media_properties/lena/video/fresh_creative_generation.py` and feeds canonical A-N source artifacts; it is not a provider compiler or execution authority.
- The competing `pipeline/video/` authority tree was removed.
- Every new Lena video requires fresh creative JSON, a freshly compiled provider prompt/request, and per-attempt immutable evidence.
- Exact prompt reuse is allowed only for the same immutable attempt's deterministic recompilation, job recovery, reconciliation, download, or validation.
- A QA-rejected attempt cannot be submitted again as a new provider create call under its old prompt.
- The SpaceX web generation job `46af7a32-8b36-46e6-adbf-b3172a635e83` is historical learning evidence only: one submitted generation call, zero retries, 20 credits charged, QA rejected, no queue or publication from that job, and SpaceX Attempt 02 cancelled/unauthorized.
- The current video slot is complete because Nic already posted a video. Do not resume SpaceX, prepare Attempt 02, or recommend another generation for that completed slot unless Nic explicitly requests a revision or replacement.
- The next open video slot requires a completely fresh concept, fresh JSON authority, and freshly compiled prompt/request. No production action is required until that next slot is open or Nic gives a new instruction.
...
Current canonical Lena video authority:

- source namespace: `pipeline/media_properties/lena/video/`
- fresh creative generator: `pipeline/media_properties/lena/video/fresh_creative_generation.py`
- canonical compiler: `pipeline/media_properties/lena/video/compiler.py`
- canonical validator: `pipeline/media_properties/lena/video/validation.py`
- canonical architecture documentation: `pipeline/media_properties/lena/video/documentation/LENA_VIDEO_JSON_PRODUCTION_SYSTEM_V1.md`
- Character Element UUID: `6a842337-ef20-4cb9-a0ff-04fa5eb8f8d3`
- direct Character Element binding: mandatory
- standard governed target: 8 seconds, 720p, 9:16, 36-credit ceiling unless a later explicit authority changes the video policy
- current execution authority: none

Fresh video authority rules:

- Lena's identity, safety policy, cost policy, and platform policy remain stable.
- Every new Lena video must receive newly generated creative JSON.
- Every new Lena video must receive a newly compiled provider prompt and request.
- Every paid provider attempt must have its own immutable attempt artifact or attempt record bound to the compiled request, authorization, attempt limit, retry limit, previous job where applicable, previous QA findings, and exact creative changes.
- A QA-rejected attempt cannot be submitted again as a new provider create call under its old prompt.
- Exact prompt reuse is allowed only for deterministic recompilation of the same immutable attempt, recovery of the same provider job, reconciliation of the same submission, download of the same result, or validation of the same result.
- The SpaceX Pilot and rejected SpaceX live result are not reusable production prompts or automatic retry instructions.
- Learning from a failed generation may influence future fresh concepts without forcing regeneration of that concept.

Operational-state reconciliation before recommendations:

- Determine what has already been generated, posted, rejected, queued, or published before recommending work or producing a command.
- Determine whether the current content slot is already complete.
- Identify which artifact is active production work and which artifacts are historical evidence only.
- Do not continue the most recent technical thread when it conflicts with actual production state.
- When Nic states that a video for the current slot has already been posted, treat that slot as complete. Do not prepare, authorize, or recommend another generation for that completed slot unless Nic explicitly requests a revision or replacement.
- The next production task after a completed slot must target the next open slot and use a fresh concept unless Nic explicitly requests a revision or replacement.
- Preserve rejected outputs and QA findings as immutable learning evidence. Do not automatically create a retry, make the rejected concept the next production task, invent blockers, or add extra review stages.
- Do not ask Nic which internal document, schema, or module owns a rule. Inspect the repository and place durable doctrine in the correct authority and temporary state in the session-start/handoff document.
- Give Nic one exact next action based on verified current state.

Historical governed provider route:
...
Historical controlled route requirements:
...
The benchmark audio remains bound to the 7.041667-second benchmark and must not be automatically reused for a future asset.

This historical benchmark proved useful facts about Character Element delivery, Lena identity stability, HPE presence, and sensual creative direction. It does not override the current canonical fresh-video authority, current 8-second video policy, or completed-slot state.

Historical code checkpoints:
...
Historical Attempt05 candidate:
...
Historical note: that candidate reused the canonical balcony scene but introduced materially new governed motion. Do not treat that old ranking result as active production work.

Historical governed execution surfaces:
...
Historical unsigned authorization request:
...
That historical Attempt05 state is no longer the next production instruction. Do not resume it, present it as the current open task, or treat it as authorization for generation.

Historical generation output root bound into the unsigned request:
...
Historical music provider route:
...
2. Preserve the approved historical audiovisual benchmark and music evidence as historical evidence.
3. Preserve PR #139's merged canonical fresh-video authority at main `824f58615eee41e15b93f72d71bb8b8241fcf169`.
4. Treat the SpaceX job `46af7a32-8b36-46e6-adbf-b3172a635e83` as historical learning evidence only: QA rejected, 20 credits charged, one submitted generation call, zero retries, no queue/publication, and Attempt 02 cancelled/unauthorized.
5. Treat the current video slot as complete because Nic already posted a video.
6. Do not resume SpaceX, prepare SpaceX Attempt 02, or recommend another generation for the completed slot unless Nic explicitly requests a revision or replacement.
7. The next open video slot, when opened by schedule or Nic instruction, requires a completely fresh concept, fresh canonical JSON authority, and freshly compiled prompt/request.
8. No production action is currently required until the next slot is open or Nic gives a new instruction.
9. Continue through generation, audio/assembly, video QA, queue, publication, recovery, metrics, and learning only through separately proven governed boundaries.
...
The dashboard is support infrastructure. It must not become the product priority over the active Lena media lane.

The current immediate task is state/document synchronization. It is not a cost query, generation, retry, queue, publication, scheduler change, live photo-lane change, or ITB change.
