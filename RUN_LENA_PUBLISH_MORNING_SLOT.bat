...
SET LOGFILE=logs\scheduler\%POST_DATE%\publish_morning_%POST_DATE%.log

echo [%DATE% %TIME%] START morning slot publish >> %LOGFILE%

.venv\Scripts\python.exe tools\lena_autopublish_approved_queue_v2_8.py ^
    --date %POST_DATE% ^
    --platforms "Instagram Feed,Facebook Page" ^
    --slot-keyword morning ^
    --max-attempts 3 ^
    >> %LOGFILE% 2>&1

SET RC=%ERRORLEVEL%
echo [%DATE% %TIME%] END morning slot publish  rc=%RC% >> %LOGFILE%
exit /b %RC%
